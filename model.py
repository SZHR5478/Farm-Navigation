...
def build_model(obs_space, action_space, args, device):
    ...
    model = A3CModel(obs_shape, action_space[0], args.rnn_out, args.network, args.stack_frames, device)
    model.train()
    return model
...
class A3C_Dueling(torch.nn.Module):
    ...
    def forward(self, inputs, test=False):
        ...
        if self.single or states.shape[0] == 1:
            ...
            return value0, [action_0], entropy_0, log_prob_0, (hx_0, cx_0), R_pred
