...
parser.add_argument('--entropy', type=float, default=0.01, metavar='E', help='parameter for entropy')
...
parser.add_argument('--env-test', default='Track2D-BlockPartialNav-v0', metavar='ENVB', help='environment to test on ')
...
if __name__ == '__main__':
    ...
    #p = mp.Process(target=test, args=(args, shared_model, n_iters))
    #p.start()
    #processes.append(p)
    ...
    for rank in range(0, args.workers):
        p = mp.Process(target=train, args=(
            rank, args, shared_model, optimizer, n_iters))
        p.start()
        processes.append(p)
        time.sleep(args.sleep_time)
    for p in processes:
